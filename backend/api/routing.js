...
const { generateOptimalRoutes, optimizeRoute, getNurseAppointmentsForDay } = require('../services/azure-routing');
...
router.post('/optimize', async (req, res) => {
  try {
    ...
    console.log(`🚀 API: Optimizing routes for ${nurseIds.length} nurses on ${date}...`);
...
router.get('/appointments/:nurseId/:date', async (req, res) => {
  try {
    ...
    console.log(`📅 API: Fetching appointments for nurse ${nurseId} on ${date}...`);
    
    const appointmentsByNurse = await getNurseAppointmentsForDay([nurseId], date);
    ...
    if (appointmentsByNurse[nurseId]) {
      ...
      res.status(404).json({
        success: false,
        error: 'No appointments found for this nurse and date, or nurse coordinates missing'
      });
    }
...
router.get('/status', async (req, res) => {
  try {
    ...
    const routablePercentage = totalAppointmentsCount > 0 
...
router.get('/nurses-with-routes/:date', async (req, res) => {
  try {
    ...
    console.log(`👩‍⚕️ API: Finding nurses with routable appointments on ${date}...`);
    ...
    res.json({
      ...
      data: {
        ...
        totalAppointments: nursesWithRoutes.reduce((sum, nurse) => sum + (nurse.appointmentCount || 0), 0)
      }
    });
...
router.post('/optimize-all-today', async (req, res) => {
  try {
    ...
    console.log(`   Found ${nurseIds.length} nurses with routable appointments`);
