...
const { eq, inArray, and, or, like, between, ne, gte, lte, isNotNull } = require('drizzle-orm');
...
router.get('/options', async (req, res) => {
  try {
    ...
    // Get unique nurses
    const nurses = await db.selectDistinct({
      id: appointments.nurseId,
      name: appointments.nurseName
    })
    .from(appointments)
    .where(and(
      isNotNull(appointments.nurseId),
      isNotNull(appointments.nurseName)
    ))
    .orderBy(appointments.nurseName);

    // Get unique patients  
    const patients = await db.selectDistinct({
    ...
    // Get unique statuses
    const statusResults = await db.selectDistinct({
    ...
    console.log(`✅ Found ${result.nurses.length} nurses, ${result.patients.length} patients, ${result.serviceTypes.length} service types`);
    res.json(result);
...
router.get('/filter', async (req, res) => {
  try {
    const {
      nurses,        // Comma-separated nurse IDs
      patients,      // Comma-separated patient IDs  
      statuses,      // Comma-separated status values
      serviceTypes,  // Comma-separated service codes
      locations,     // Comma-separated location IDs
      dateFrom,      // Start date (YYYY-MM-DD)
      dateTo,        // End date (YYYY-MM-DD)
      hasCoordinates // true/false - only appointments with lat/lng
    } = req.query;

    console.log('🔍 Filtering appointments with:', { nurses, patients, statuses, dateFrom, dateTo });
    ...
    if (dateFrom || dateTo) {
      if (dateFrom && dateTo) {
        conditions.push(between(appointments.startDate, dateFrom, dateTo + ' 23:59:59'));
...
router.get('/mappable', async (req, res) => {
  try {
    ...
    console.log(`✅ Found ${results.length} mappable appointments for ${Object.keys(appointmentsByNurse).length} nurses`);
...
router.get('/calendar', async (req, res) => {
  try {
    ...
    console.log(`📅 Fetching calendar data from ${dateFrom} ${dateTo ? `to ${dateTo}` : ''}`);
    ...
    const groupedByDate = results.reduce((acc, appointment) => {
      const date = appointment.startDate?.split(' ')[0] || appointment.startDate?.split('T')[0];
      if (!acc[date]) {
        acc[date] = [];
      }
    ...
    console.log(`✅ Found ${results.length} appointments across ${Object.keys(groupedByDate).length} dates`);
...
router.get('/stats', async (req, res) => {
  try {
    ...
    // By nurse
    const nurseCounts = await db.select({
      nurseName: appointments.nurseName,
      nurseId: appointments.nurseId,
      count: sql`count(*)`
    })
    ...
    console.log(`✅ Statistics: ${stats.total} total appointments, ${stats.byNurse.length} nurses`);
...
router.post('/sync', async (req, res) => {
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => reject(new Error('Sync timeout after 10 minutes')), timeoutMs);
    });
    ...
    if (result.success) {
      console.log(`✅ Sync completed successfully: ${result.count} appointments`);
      res.json({
        success: true,
...
router.get('/sync/status', async (req, res) => {
  try {
    ...
    res.json({
      ...
      lastSyncApproximate: lastUpdated[0]?.lastUpdate ? 'Recently synced' : 'No data',
      timestamp: new Date().toISOString()
    });
